      ...
      <Card className="flex-1 overflow-hidden">
        <CardContent className="p-4 h-full flex items-center justify-center bg-[#f5f5f5]">
          ...
          >
            ...
            {/* Product Mockup with Color Tint - Using CSS masking for color overlay */}
            {state.product && (
              <div className="absolute inset-0 flex items-center justify-center">
                {/*
                  Color overlay technique:
                  1. The SVG mockup has a transparent background and white garment areas
                  2. We use CSS masking to apply color only to the garment shape
                  3. The original mockup is overlaid with multiply blend mode for texture
                */}
                <div className="relative w-full h-full">
                  {(() => {
                    const currentView = state.product.views.find((v: any) => v.angle === state.view);
                    const mockupUrl = currentView?.mockupUrl || state.product.thumbnailUrl;
                    const maskUrl = currentView?.maskUrl || mockupUrl;

                    return (
                      <>
                        {/* Color tint layer - uses enhanced mask for complete solid coverage */}
                        <div
                          className="absolute inset-0"
                          style={{
